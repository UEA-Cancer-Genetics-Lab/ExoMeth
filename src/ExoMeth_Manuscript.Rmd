...
# source(here("src/Boruta_Computation.R"))

# The pre-computed Boruta resamples can be loaded in for analysis here:
### WARNING: THIS WILL CRASH A BINDER INSTANCE AS IT IS LARGER THAN THE ALLOWED MEMORY (1 to 2 GB)! ###
# ResampledBoruta <- readRDS(here("output/data_out/ResampledBoruta.RDS"))

# Instead we load the pre-processed Feature Importances and and confirmed variable objects to save memory 
# and for ease of working.
ConfirmedVariables <- readRDS(here("output/data_out/ConfirmedVariables.RDS"))
FeatureImportances <- readRDS(here("output/data_out/FeatureImportances.RDS"))
```
...
BorutaPlotData <- FeatureImportances$ExoMeth %>%
...
BorutaPlot <- ggplot(BorutaPlotData, 
  ...
  guides(
    fill = guide_legend(
      title = 'Proportion of resamples "Confirmed":',
      title.position = "top",
      title.hjust = 0.5
    )) +
  labs(
...
ggsave(
  filename = here("output/fig_out/Figure 3 (Density).pdf"),
...
EstimationPlot
ggsave(
  filename = here("output/fig_out/Figure 4 (Estimation).pdf"),
  height = 6, width = 9, units = "in",
  device = cairo_pdf
)
```
...
ggsave(
  filename = here("output/fig_out/Figure 5 (DCA).pdf"),
...
Reduction <- map2(
  ...
  function(outcome, outcome_name) {
    outcome %>%
      ...
      mutate(
        SoCReduction = (SoC - All) * 100 / (Threshold / (1 - Threshold)),
...
ggsave(
  filename = here("output/fig_out/Figure 6 (Reduction).pdf"),
  height = 10, width = 13, units = "in",
  device = cairo_pdf
)
```
...
ggplot(FeatureImportances$SoC, aes(x = reorder(Variable, Importance), y = Importance, fill = as.factor(Proportion))) +
  geom_boxplot(outlier.alpha = 0.5) +
  theme_pubclean() +
  theme(
    legend.position = "bottom",
    axis.text.y = element_text(
      face = case_when(
        levels(reorder(FeatureImportances$SoC$Variable, 
                       FeatureImportances$SoC$Importance)) %in% str_remove(MethProbes, "^m") ~ "italic",
        levels(reorder(FeatureImportances$SoC$Variable, 
                       FeatureImportances$SoC$Importance)) %in% NanoGenes ~ "bold",
        levels(reorder(FeatureImportances$SoC$Variable, 
                       FeatureImportances$SoC$Importance)) %in% ClinVars ~ "bold.italic",
        TRUE ~ "plain"
      )
    )
  ) +
...
ggplot(FeatureImportances$Methylation, aes(x = reorder(Variable, Importance), y = Importance, fill = as.factor(Proportion))) +
  geom_boxplot(outlier.alpha = 0.5) +
  theme_pubclean() +
  theme(
    legend.position = "bottom",
    axis.text.y = element_text(
      face = case_when(
        levels(reorder(FeatureImportances$Methylation$Variable, 
                       FeatureImportances$Methylation$Importance)) %in% str_remove(MethProbes, "^m") ~ "italic",
        levels(reorder(FeatureImportances$Methylation$Variable, 
                       FeatureImportances$Methylation$Importance)) %in% NanoGenes ~ "bold",
        levels(reorder(FeatureImportances$Methylation$Variable, 
                       FeatureImportances$Methylation$Importance)) %in% ClinVars ~ "bold.italic",
        TRUE ~ "plain"
      )
    )
  ) +
...
# Instead of calling the built-in plot function, spend way too much effort making it prettier through ggplot:
ggplot(FeatureImportances$ExoRNA, aes(x = reorder(Variable, Importance), y = Importance, fill = as.factor(Proportion))) +
  geom_boxplot(outlier.alpha = 0.5) +
  theme_pubclean() +
  theme(
    legend.position = "bottom",
    axis.text.y = element_text(
      face = case_when(
        levels(reorder(FeatureImportances$ExoRNA$Variable, 
                       FeatureImportances$ExoRNA$Importance)) %in% str_remove(MethProbes, "^m") ~ "italic",
        levels(reorder(FeatureImportances$ExoRNA$Variable, 
                       FeatureImportances$ExoRNA$Importance)) %in% NanoGenes ~ "bold",
        levels(reorder(FeatureImportances$ExoRNA$Variable, 
                       FeatureImportances$ExoRNA$Importance)) %in% ClinVars ~ "bold.italic",
        TRUE ~ "plain"
      )
    )
  ) +
...
TriSig_RF_Density_Plots <- map2(
  ...
  function(ModelResult, title) {
    suppressMessages(Model_Plots(ModelResult$Results,
      ...
      scale_fill_manual(
        values = c(D3Green, D3Orange, D3Purp),
        labels = c(
          "NEC",
...
ggarrange(
  plotlist = TriSig_RF_Density_Plots,
  common.legend = TRUE,
  labels = "AUTO",
  font.label = 28,
  legend = "bottom",
